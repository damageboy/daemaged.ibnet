...
namespace Daemaged.IBNet
{
  ...
  internal class EchoStream : Stream
  {
    ...
    /// <summary>
    /// Gets or Sets the ReadTimeout of the primary stream.
    /// The slave's timeout is unchanged;
    /// </summary>
    ...
    /// <summary>
    /// Gets or Sets the WriteTimeout of the primary stream.
    /// The slave's timeout is unchanged;
    /// </summary>
    ...
    public override int Read(byte[] buffer, int offset, int count)
    {
      ...
      m_lastReadResult = _primaryStream.Read(buffer, offset, count);
      if (m_lastReadResult != 0) {
        ...
        if (m_readFailAction == SlaveFailAction.Propogate) {
          ...
          _slaveStream.Write(buffer, offset, m_lastReadResult);
        }
        else {
          // This is the case that involves more expensive error
          // handling.
